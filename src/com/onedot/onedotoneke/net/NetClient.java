package com.onedot.onedotoneke.net;
...
import com.onedot.onedotoneke.App;
import com.onedot.onedotoneke.Constants;
...
public class NetClient {
	...
	// Imageload缓存目录
	private static File cacheDir = new File(App.getAppCacheDir() + "/Imageload");
	...
	static {
		ActivityManager am = (ActivityManager) App.sharedApplication().getContext()
				.getSystemService(Context.ACTIVITY_SERVICE);
		int memClass = am.getMemoryClass();
		int cacheSize = 1024 * 1024 * memClass / 4; // 硬引用缓存容量，为系统可用内存的1/4

		binner_options = new DisplayImageOptions.Builder()
				.showImageOnLoading(R.drawable.ic_launcher) // resource
				// or
		...
		ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(
				App.sharedApplication().getContext()).threadPoolSize(10)
